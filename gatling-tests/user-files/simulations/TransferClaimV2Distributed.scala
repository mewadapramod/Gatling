...
class TransferClaimV2Distributed extends Simulation {
  ...
  var myScenario = scenario("Transfer Claim V1")
    .during(dur seconds) {
      exitBlockOnFail(
...
var myScenario2 = scenario("Transfer Claim V2")
    .during(dur seconds) {
      exitBlockOnFail(
          exec(flushSessionCookies)
         .exec(flushCookieJar)
   ...
   if(dur == 0) {
      ...
      val run = myScenario2.inject(
         //rampUsers(users) over (rps seconds)
         splitUsers(users) into (rampUsers(rampedUsers) over (rps seconds)) separatedBy (delay seconds)
      )
      // val run2=  myScenario2.inject(
      //    //rampUsers(users) over (rps seconds)
      //    splitUsers(divide(users, 2)) into (rampUsers(divide(rampedUsers, 2)) over (rps seconds)) separatedBy (delay seconds)
      // )
