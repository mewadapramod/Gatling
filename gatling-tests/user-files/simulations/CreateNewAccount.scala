...
class CreateNewAccount extends Simulation {
  ...
  var myScenario = scenario("Create New Account")
    .during(dur seconds) {
      exitBlockOnFail(
         exec(flushSessionCookies)
         .exec(flushCookieJar)
         .exec(flushHttpCache)
         .exec(CreateAccount.createUser)
         .exec(Logout.logout)
         .exec(NewUserLogin.login)
         .pause(1000 milliseconds, 5000 milliseconds)
      )
   }

   if(dur == 0) {
      myScenario = scenario("Create New Account")
      .exitBlockOnFail(
         exec(flushSessionCookies)
         .exec(flushCookieJar)
         .exec(flushHttpCache)
