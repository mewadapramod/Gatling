...
class ValidateHoldseat extends Simulation {
  ...
  var myScenario = scenario("User goes to review cart with 5 seats")
    .during(dur seconds) {
      exitBlockOnFail(
   ...
   if(dur == 0) {
      myScenario = scenario("User goes to review cart with 5 seats")
      .exitBlockOnFail(
         exec(flushSessionCookies)
         .exec(flushCookieJar)
         .exec(flushHttpCache)
