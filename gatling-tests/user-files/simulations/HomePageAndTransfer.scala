...
class HomePageAndTransfer extends Simulation {
  ...
  var myScenario = scenario("Home Page")
    .during(dur seconds) {
      exitBlockOnFail(
         ...
         .pause(1000 milliseconds, 1000 milliseconds)
      )
   }

   val myScenario2 = scenario("Transfer Claim")
    .during(dur seconds) {
      exitBlockOnFail(
         exec(flushSessionCookies)
         .exec(flushCookieJar)
         .exec(flushHttpCache)
         .exec(LoadHomePageReferer.getHomePage)
   ...
   if(dur == 0) {
      ...
      val run2=  myScenario2.inject(
         //rampUsers(users) over (rps seconds)
         splitUsers(divide(users, 20)) into (rampUsers(divide(rampedUsers, 20)) over (rps seconds)) separatedBy (delay seconds)
      )
