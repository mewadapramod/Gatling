...
class InvoicePayments extends Simulation {
  ...
  var myScenario = scenario("Invoice Payments")
    .during(dur seconds) {
      exitBlockOnFail(
         ...
         .exec(PaymentPlans.getPaymentPlans)
         .exec(CCQuery.getCCQuery)
         .exec(PayInvoice.payment)
  ...
  if(dur == 0) {
      myScenario = scenario("Invoice Payments")
         .exitBlockOnFail(
            exec(flushSessionCookies)
            .exec(flushCookieJar)
            .exec(flushHttpCache)
