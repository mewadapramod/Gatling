...
class OverviewUpgrade extends Simulation {
  ...
  var myScenario = scenario("Checking complete venue availability")
    .during(dur seconds) {
      exitBlockOnFail(
        ...
        // .feed(() => xtransid)
         ...
         .exec(InitializeUpdateRequest.getInitializeXtransId)
         .exec(VenueConfigUpdateRequest.getVenueConfigurationData)
         .exec(PriceDataUpgradeRequest.getPriceData)
         .exec(VenueAvailabilityUpgradeRequest.getVenueAvailabilityData)
        // .exec(LoadMapboxPage.getMapboxPage)
         .exec(CameraCategoryUpgradeRequest.getCameraCategoryData)
         .exec(SectionDataUpgradeRequest.getSectionDataOfGroup1)
         .exec(SectionAvailabilityUpgradeRequest.getSectionAvailabilityDataOfGroup1)
   ...
   if(dur == 0) {
      myScenario = scenario("Checking complete venue availability")
      .exitBlockOnFail(
         exec(flushSessionCookies)
         .exec(flushCookieJar)
         .exec(flushHttpCache)
