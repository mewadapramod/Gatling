...
class InvoiceNew extends Simulation {
...
//  .inferHtmlResources(/*BlackList("""^https://static-shard1\.io-media\..*""","""^https://am-static-01\.ticketmaster\..*""","""^https://.*\.googleapis\..*""")*//*, WhiteList("""^http://.*\.io-media.com/.*""")*/)

  var myScenario = scenario("Invoice Load Time")
    .during(dur seconds) {
      exitBlockOnFail(
         ...
         .exec(LoadInvoice.getInvoice)
         .exec(InvoiceList.getInvoiceList)
         .exec(InvoiceDetails.getInvoiceDetails)
         .exec(Logout.logout)
  ...
  if(dur == 0) {
      myScenario = scenario("Invoice Load Time")
         .exitBlockOnFail(
            exec(flushSessionCookies)
            .exec(flushCookieJar)
            .exec(flushHttpCache)
