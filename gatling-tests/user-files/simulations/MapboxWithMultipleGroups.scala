...
class MapboxWithMultipleGroups extends Simulation {
  ...
  var myScenario = scenario("User goes to map box with multiple groups")
    .during(dur seconds) {
      exitBlockOnFail(
        ...
        // .feed(() => xtransid)
         ...
         .exec(SectionDataPostRequest.getSectionDataOfGroup2)
         .exec(SectionAvailabilityPostRequest.getSectionAvailabilityDataOfGroup2)
         .exec(SectionDataPostRequest.getSectionDataOfGroup3)
         .exec(SectionAvailabilityPostRequest.getSectionAvailabilityDataOfGroup3)
   ...
   if(dur == 0) {
      myScenario = scenario("User goes to map box with multiple groups")
      .exitBlockOnFail(
         exec(flushSessionCookies)
         .exec(flushCookieJar)
         .exec(flushHttpCache)
