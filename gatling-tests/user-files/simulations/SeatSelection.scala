...
class SeatSelection extends Simulation {
  ...
  var myScenario = scenario("User goes to map box and select a seat")
    .during(dur seconds) {
      exitBlockOnFail(
   ...
   if(dur == 0) {
      myScenario = scenario("User goes to map box and select a seat")
      .exitBlockOnFail(
         exec(flushSessionCookies)
         .exec(flushCookieJar)
         .exec(flushHttpCache)
