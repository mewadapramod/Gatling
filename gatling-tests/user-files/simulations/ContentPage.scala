...
class ContentPage extends Simulation {
  ...
  var myScenario = scenario("Load Content Page")
    .during(dur seconds) {
      exitBlockOnFail(
         exec(flushSessionCookies)
         .exec(flushCookieJar)
         .exec(flushHttpCache)
         .exec(LoadHomePagePublic.getHomePage)
         //.exec(LoadLiveEventLayoutA.getLiveEventLayoutA)
         //.exec(LoadLiveEventLayoutB.getLiveEventLayoutB)
         //.exec(LoadPageLayoutB.getLayoutB)
         //.exec(LoadArtsLayoutA.getArtsA)
         //.exec(LoadArtsLayoutB.getArtsB)
         //.exec(LoadLayoutC.getLayoutC)
         .pause(5000 milliseconds, 10000 milliseconds)
      )
   }

  if(dur == 0) {
      myScenario = scenario("Load Content Page")
         .exitBlockOnFail(
         exec(flushSessionCookies)
         .exec(flushCookieJar)
         .exec(flushHttpCache)
      ...
      //   .exec(LoadLiveEventLayoutA.getLiveEventLayoutA)
        // .exec(LoadLiveEventLayoutB.getLiveEventLayoutB)
         //.exec(LoadPageLayoutB.getLayoutB)
         //.exec(LoadArtsLayoutA.getArtsLayoutA)
         //.exec(LoadArtsLayoutB.getArtsB)
         //.exec(LoadLayoutC.getLayoutC)
            .pause(5000 milliseconds, 10000 milliseconds)
         )
