...
class MapboxWithPromoCode extends Simulation {
  ...
  var myScenario = scenario("User goes to map box and apply promocode")
    .during(dur seconds) {
      exitBlockOnFail(
   ...
   if(dur == 0) {
      myScenario = scenario("User goes to map box and apply promocode")
      .exitBlockOnFail(
         exec(flushSessionCookies)
         .exec(flushCookieJar)
         .exec(flushHttpCache)
