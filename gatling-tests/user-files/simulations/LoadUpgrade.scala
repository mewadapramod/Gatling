...
class LoadUpgrade extends Simulation {
  ...
  var myScenario = scenario("Queue Page")
    .during(dur seconds) {
      exitBlockOnFail(
         exec(flushSessionCookies)
         .exec(flushCookieJar)
         .exec(flushHttpCache)
         .exec(LoadUpgradePage.getUpgradePage)
         .pause(5000 milliseconds, 10000 milliseconds)
         .exec(session => {
                         val response = session("queue_number").as[String]
                         println("Queue number in Response body: \n$response")
                         session
                       })
         )
   }
  ...
  if(dur == 0) {
      myScenario = scenario("Queue Page")
         .exitBlockOnFail(
            exec(flushSessionCookies)
            .exec(flushCookieJar)
            .exec(flushHttpCache)
