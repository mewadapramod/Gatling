...
class SeatSelectionWithPromoCode extends Simulation {
  ...
  var myScenario = scenario("User goes to map box applies promo code and select a seat")
    .during(dur seconds) {
      exitBlockOnFail(
        ...
        // .feed(() => xtransid)
         ...
         .exec(PriceDataPostRequest.getPriceDataUsingPromocode)
         .exec(VenueAvailabilityPostRequest.getVenueAvailabilityDataUsingPromocode)
         ...
         .exec(SectionAvailabilityPostRequest.getSectionDataUsingPromocodeOfGroup1)
   ...
   if(dur == 0) {
      myScenario = scenario("User goes to map box applies promo code and select a seat")
      .exitBlockOnFail(
         exec(flushSessionCookies)
         .exec(flushCookieJar)
         .exec(flushHttpCache)
