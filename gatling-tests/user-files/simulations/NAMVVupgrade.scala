...
class NAMVVupgrade extends Simulation {
 ...
 // .inferHtmlResources(/*BlackList("""^https://.*\.cloudfront\..*""")*//*, WhiteList("""^https://amgr-tm-qa.s3.amazonaws.com/.*""")*/)
  ...
  .during(dur seconds) {
      exitBlockOnFail(
         ...
         .exec(UpgradeList.getUpgradelist)
         .exec(VVMemberEvents.getVVAllEvents)
         .exec(MemberList.getMembers)
         .exec(TransferUpgradetoVV.TransfertoVV)
       //   .exec(BuyMemberList.TransfertoVV)
